(function () {
    ...
    info.innerHTML = '<br><div>Created by Rath</div><br><div>V2.3</div>';

    function getTotalUnits(unit) {
        const input = document.querySelector(`#unit_input_${unit}`);
        if (input && input.parentElement) {
            const total = input.parentElement.textContent.match(/\((\d+)\)/);
    ...
    sendBtn.onclick = () => {
        ...
        function enviarAtaque() {
            ...
            units.forEach(([unit]) => {
                ...
                const formInput = form[unit];
                if (!formInput) return;

                let qty = checkbox.checked
                    ? getTotalUnits(unit)
                    : parseInt(input.value) || 0;

                if (qty > 0) {
                    setFieldValue(formInput, qty);
                    total += qty;
                }
            });

            if (total === 0) {
                console.log(`Sem tropas suficientes para ${x}|${y}, a saltar...`);
                currentCoord++;
                rep = 0;
                setTimeout(enviarAtaque, 500);
                return;
            }
